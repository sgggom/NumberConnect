...
5. **结果可复现**：相同路径、参数和种子必须得到相同结果，方便关卡复盘、批量生成和跨端校验。
...
编辑器、批量生成表和难度配置都把隐藏比例视为明确输入。如果算法因局部约束提前停止，同一配置会产生不同隐藏数量，关卡统计和策划验收都会失去稳定口径。因此算法选择“数量确定、形态尽量接近约束”，而不是“形态绝对合规、数量不确定”。
...
| 基础隐藏占比为 0 | 仍会加上难度百分点；较大棋盘可能产生隐藏格 |
| 基础隐藏占比与难度之和超过 100 | 实际占比截断为 100%，但首尾仍显示 |
...
若 C# 端需要批量生成大量大棋盘，建议先保持逻辑一致完成验收，再考虑以下无行为变化的优化：
