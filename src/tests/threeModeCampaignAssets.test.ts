...
describe('三模式正式关卡资产', () => {
  it('完整覆盖500关配置、全部阶段引用和1–10档难度', () => {
    ...
    expect(campaign).toHaveLength(500);
    expect(library.levels.size).toBe(15350);
    expect(library.guides.size).toBe(37);
    expect(campaign[0].stages).toHaveLength(2);
    expect(campaign[1].stages).toHaveLength(3);
    expect(campaign[499].stages).toHaveLength(4);
    expect(resolveThreeModeStage(library, campaign[0], { stage: 1 }).formationId)
      .toBe('guide_41_1');
    expect(resolveThreeModeStage(library, campaign[499], {
      stage: 4,
      targetDifficulty: 10,
    }).formationId).toBe('level_810_25_10');
  });
});
