...
describe('editor level play simulation', () => {
  ...
  it('formats one row of level base data for spreadsheet paste', () => {
    const row = formatLevelBaseDataTsv({
      ...
      metrics: {
        ...
        rightAngleTurnRatio: 8 / 34,
        acuteAngleTurnRatio: 3 / 34,
        obtuseAngleTurnRatio: 4 / 34,
        averageSegmentLength: 2.25,
        upwardMoveRatio: 0.125,
        downwardMoveRatio: 0.125,
        leftwardMoveRatio: 0.125,
        rightwardMoveRatio: 0.125,
        upperLeftMoveRatio: 0.125,
        upperRightMoveRatio: 0.125,
      ...
      averageStepDifficultyScore: 7.777,
  ...
  it('summarizes whole, early, middle, and late difficulty scores', () => {
    expect(summarizeDifficultyScores([1, 3, 5, 7, 9, 11, 13, 15])).toEqual({
      averageStepDifficultyScore: 8,
      earlyAverageDifficultyScore: 2,
      middleAverageDifficultyScore: 8,
      lateAverageDifficultyScore: 14,
    });
    expect(summarizeDifficultyScores([])).toEqual({
