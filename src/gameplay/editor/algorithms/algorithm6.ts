...
export const createAlgorithm6Selection = (): Algorithm6Selection => ({
  id: 'algorithm-6',
  parameters: {
...
type Algorithm6Phase = 'early' | 'middle' | 'late';

interface Algorithm6PhaseSettings {
  hiddenProbability: number;
  rowColumnHiddenSkipProbability: number;
}

const phaseForPathIndex = (index: number, pathCount: number): Algorithm6Phase => {
...
const phaseSettingsFor = (
  ...
  phase: Algorithm6Phase,
): Algorithm6PhaseSettings => {
  if (phase === 'early') {
    return {
...
export const calculateAlgorithm6RowColumnHiddenSkipProbability = (
...
export const findAlgorithm6RowColumnPeerIndexes = (
...
export interface Algorithm6HiddenSelection {
  hiddenCells: Set<string>;
}

export const selectAlgorithm6HiddenLayout = (
  path: ReadonlyArray<Cell>,
  _shape: EditorShape,
  selection: Algorithm6Selection,
  seed: number,
): Algorithm6HiddenSelection => {
  ...
  const rowColumnPeers = path.map((_, index) => (
    findAlgorithm6RowColumnPeerIndexes(path, index)
  ));
  ...
  for (let index = 1; index < path.length - 1; index += 1) {
    ...
    const alignedHiddenCount = !mustHideForVisibleLimit && selectedByProbability
      ...
      : 0;
    const actualSkipProbability = calculateAlgorithm6RowColumnHiddenSkipProbability(
      rowColumnHiddenSkipProbability,
      alignedHiddenCount,
...
export const runAlgorithm6 = (
  ...
  const seed = Math.imul(context.generationIndex + 1, 104729)
    ...
    ^ path.length
    ^ 0x4f1bbcdc;
  const hiddenCells = selectAlgorithm6HiddenLayout(path, context.shape, selection, seed);
