...
describe('level editor path generation', () => {
  it('uses algorithm 1 and the requested generation defaults for a new editor level', () => {
    const model = new LevelEditorModel();

    expect(model.algorithmSelection.id).toBe('algorithm-1');
  ...
  it('algorithm 1 saves a fixed hidden layout without enforcing uniqueness', () => {
    const model = new LevelEditorModel();
    ...
    expect(level?.algorithm?.id).toBe('algorithm-1');
    expect(level?.algorithm?.parameters.turnProbability).toBe(40);
    expect(level?.algorithm?.parameters.hiddenPercent).toBe(65);
    expect(level?.algorithm?.parameters.pathMode).toBe('spatial-distribution-multiple-solutions');
