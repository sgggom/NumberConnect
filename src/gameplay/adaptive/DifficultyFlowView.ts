...
export class DifficultyFlowView {
  ...
  constructor(private readonly root: HTMLElement) {
    root.innerHTML = `<div class="level-debug-card__heading"><div><span id="difficulty-flow-stage">实时处理</span><h2>动态难度状态机</h2></div><b id="difficulty-flow-status" role="status"></b></div>
      ...
      <div class="difficulty-flow-branches"><span data-branch="advance">完成 → ① 读取状态 · 开启下一轮</span><span data-branch="retry">失败／复活 → ⑤ 原棋盘重试</span></div>
  ...
  render(input: FlowInput, event?: 'selection' | 'result' | 'retry'): void {
    ...
    const values = [
      ...
      ['当前棋盘锁定', entry ? `第 ${entry.selection.difficulty} 档${entry.excluded ? '（调试覆盖）' : ''}` : '不参与动态选档'],
      ['首次能力评价', entry ? entry.measured ? '已评价 · 不重复计分' : '未评价' : '不计分'],
    ...
    records.slice(-5).reverse().forEach((record, index) => {
      ...
      li.textContent = `关 ${level} · 阶段 ${stage} · ${outcomes[record.outcome]} ｜ 能力 ${record.skillBefore.toFixed(3)} → ${record.skillAfter.toFixed(3)} ｜ 受挫 ${previous ? previous.stress + ' → ' : records.length < 200 ? '0 → ' : ''}${record.stress}${record.evidenceUsed ? '' : ' · 不重复评价能力'}`;
      history.append(li);
    });
